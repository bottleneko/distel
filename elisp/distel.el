...
(defun distel-inferior-machine ()
  ...
  (let* ((name (projectile-project-name))
         ...
         (path (format "%s_build/default/lib/*/ebin" root))
         (erlname (format "distel_%s@localhost" name))
         (bufname (format "*distel %s*" name))
         (erlcmd (format "cd %s && erl -sname %s -pa %s" root erlname path)))
    (setq inferior-erlang-buffer-name bufname)
    (inferior-erlang erlcmd)
